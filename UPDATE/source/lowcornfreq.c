...
/* THIS IS WHAT THE FNCTION DOES!!!
   Expected call: LowCornFreq $distance $etime $npts $filename $mag $delta
   $etime is under the O header in the sac header
...
int main( int argc, char *argv[]) {
	...
	int ii;
	int Tint;
//Make sure the start event time is within the signal
//Make sure the distance is positive
	if ( etime < 0.0 || etime > npts*dt ){ 
		fprintf(stderr,"The record does not ample pre event noise.\n");
		printf("%d\t%d\n",-1,-1);
		return 1;
	}
...
// --- Take the zero padded fft of the sample and the pre event noise 
	int p2 = pow(2,cp2(npts));
	Tint = (int)((etime + dist/7.0)/dt);
	int pnoise = pow(2,cp2(Tint));
	float tdata[p2];
	float noise[pnoise];
	...
	for (ii= Tint; ii<pnoise; ++ii) noise[ii]=0;
...
/* --- Begin Konno Omachi Smoothing --- */
	...
	float smooth_param = 0.20; // This is the konno omachi smoothing param
...
//	float fmax = 10.0;
	...
	if (sameFreq){ // Declared online 157
